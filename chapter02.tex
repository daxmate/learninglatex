...
\begin{texcode}[listing only, label={code:hello}, nameref={Hello World示例代码}]
...
\begin{texcode}[label={code:comments}]
	这是可见文本
...
\begin{itemize}[nosep]
	\item \texinline{equation} 和 \texinline{equation*}，用单行公式，星号的版本不编号
	      \begin{texcode}[listing and text, righthand width=5cm, lower separated=true]
\begin{equation}
	  \sum_{l=0}^{n}(n-l)^{l}
\end{equation}
		\end{texcode}
